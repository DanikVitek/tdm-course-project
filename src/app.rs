...
pub fn App() -> Html {
    ...
    let min_transport_per_line =
        use_state_eq::<DVector<f64>, _>(|| DVector::from_column_slice(&[300., 200., 1000., 500.]));
    let ships_count_per_type =
        use_state_eq::<RowDVector<u16>, _>(|| RowDVector::from_row_slice(&[50, 20, 30]));
    ...
    let response = use_state::<Result<(DMatrix<f64>, f64), Option<AttrValue>>, _>(|| Err(None));
